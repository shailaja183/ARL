...
FLAGS = flags.FLAGS
flags.DEFINE_bool('no_pretraining', False, 'model with no pretraining')
flags.DEFINE_bool('batch', False, 'use batch evaluation')
flags.DEFINE_bool('batch_increasing', False, 'batch evaluation with larger and larger data sizes')
flags.DEFINE_string('correctness_log', None, 'log file for correct predictions')
flags.DEFINE_integer('flattened_message_size', 600, 'number of continuous dimensions')
...
if __name__ == '__main__':
    FLAGS(sys.argv)
    dataset.load()
    if FLAGS.no_pretraining:
        Model = nopretrain.Model
    else:
        if not pretrain.saved_model_exists():
            print('no specified pretrained model found, running pretraining')
            pretrain.train()
